...
async function getCartItem(cart_id) {
  try {
    ...
    const [result1, fields] = await db.pool.query(target);
    console.log(result1);

    let target2 = `SELECT SUM(price) AS totalPrice FROM product LEFT JOIN cart_item USING(product_id) WHERE cart_id = ${cart_id}`;
    const [result2,field] = await db.pool.query(target2);
    if (result2[0].totalPrice === null) {
      result2[0].totalPrice = 0 ;
    };
