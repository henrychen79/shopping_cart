...
async function addProduct(data) {
  try {
    let target1 = `INSERT INTO productDetail ( category, productNum, detail, inventory, img) VALUES ('${data.category}','${data.productNum}','${data.detail}','${data.inventory}','${data.img}')`;

    let target2 = `INSERT INTO product ( productNum, category, productName, price) VALUES ('${data.productNum}','${data.category}','${data.productName}','${data.price}')`;
...
async function update_product(data) {
  try {
    ...
    let target1 = `UPDATE productDetail AS pD SET pD.category = '${data.category}',pD.productNum = '${data.productNum}',pD.detail = '${data.detail}',pD.inventory = '${data.inventory}',pD.img = '${data.img}' WHERE pD.category = '${data.category}' AND pD.productNum = '${data.productNum}'`;
    ...
    let target2 = `UPDATE product AS p
            SET p.productNum = '${data.productNum}', 
                ...
                p.price = '${data.price}'
            WHERE p.category = '${data.category}' AND p.productNum = '${data.productNum}'`;
...
async function update_inventory(product_id, newInventory) {
  try {
    let target = `UPDATE productDetail AS pD SET pD.inventory = '${newInventory}' WHERE pD.category = (SELECT category FROM product WHERE product_id = ${product_id}) AND pD.productNum = (SELECT productNum FROM product WHERE product_id = ${product_id})`;
    console.log(target);
    await global.db_pool.query(target);
    return "更新庫存量成功";
  } catch (error) {
